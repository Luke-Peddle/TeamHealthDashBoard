      ...
      data.forEach(async (file) => {
        try {
          const newCodeReviewRecord = {
            ...
            prs_reviewed: file.prs_reviewed,
            avg_review_time_hours: file.avg_review_time_hours,
          }
          console.log(newCodeReviewRecord)

          await axios.post('http://localhost:4000/api/codeReview', newCodeReviewRecord)
        }
        catch {
  ...
  return (
    <div className="relative">
      <div >
        ...
        {/* {status.message && (
          <div className={`mt-4 px-4 py-2 rounded text-sm ${status.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {status.message}
          </div>
        )} */}
      </div>
